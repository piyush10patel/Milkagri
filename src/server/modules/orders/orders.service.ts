...
export async function generateOrdersForDate(targetDate: Date): Promise<GenerateOrdersSummary> {
  ...
  // Get parent IDs of active children first to avoid batch boundary issues
  const children = await prisma.subscription.findMany({
    where: { status: 'active', parentSubscriptionId: { not: null } },
    select: { parentSubscriptionId: true },
  });
  const parentIdsWithActiveChildren = new Set(
    children.map(c => c.parentSubscriptionId)
  );
  ...
  while (hasMore) {
    ...
    for (const sub of subscriptions) {
      ...
      const holdInputs = sub.vacationHolds.map((h: any) => ({
      ...
      try {
        await prisma.deliveryOrder.create({
          data: {
            ...
            packs: subPacks.length
              ? {
                  create: subPacks.map((pack: any) => ({
